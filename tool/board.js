function Board() {
  ...
  return {
        ...
        for (var j = 0; j != LINE_COUNT; ++j) {
          ...
          if (node) {
            if (node.color == -1) { // text
              ...
              ctx.stroke();
              ctx.fillStyle = node.color == 1 ? 'black' : 'white';
              ctx.fill();

              if (node.note) {
                ctx.fillStyle = node.color == 1 ? 'white' : 'black';
                ctx.lineWidth = 2;
                ctx.fontSize = stone_size + 2;
                ctx.textBaseLine = 'middle';
                ctx.textAlign = 'center';
    ...
    generateImage: function(obj) {
      ...
      var target_ctx = document.getElementById(tmp_canvas_id).getContext("2d");

      // draw lines
      target_ctx.strokeStyle = 'black';
      target_ctx.lineWidth = 1;
        ...
        for (var j = 0; j != LINE_COUNT; ++j) {
          ...
          if (node) {
            if (node.color == -1) { // text
              ...
              target_ctx.fillStyle = 'black';
              target_ctx.lineWidth = 2;
              target_ctx.fontSize = stone_size + 2;
              target_ctx.textBaseLine = 'middle';
              target_ctx.textAlign = 'center';
              ...
              target_ctx.stroke();
              target_ctx.fillStyle = node.color == 1 ? 'black' : 'white';
              target_ctx.fill();

              if (node.note) {
                target_ctx.fillStyle = node.color == 1 ? 'white' : 'black';
                target_ctx.lineWidth = 2;
                target_ctx.fontSize = target_stone_size + 2;
                target_ctx.textBaseLine = 'middle';
                target_ctx.textAlign = 'center';
